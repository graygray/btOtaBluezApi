...
static int store_device_info(const bdaddr_t *src, const bdaddr_t *dst, struct hidp_connadd_req *req)
{
	...
	LOGE("======Tina======== store_device_info 3 err:%d  ERR:%s\n", err, strerror(errno));
	return err;
}
...
{
	LOGE("======Tina======== get_sdp_device_info 0\n");
	...
	LOGE("======Tina======== get_sdp_device_info 1\n");
	...
	LOGE("======Tina======== get_sdp_device_info 2\n");
	if (pnp_rsp) {
	...
	LOGE("======Tina======== get_sdp_device_info 3\n");
	if (bacmp(&bdaddr, BDADDR_ANY)){
		LOGE("======Tina======== get_sdp_device_info 4\n");
		store_device_info(&bdaddr, dst, req);
	}
	LOGE("======Tina======== get_sdp_device_info 5\n");
...
void bnep_sdp_unregister(void)
{
	if (record && sdp_record_unregister(session, record))
		LOGE("Service record unregistration failed.");
...
int bnep_sdp_register(bdaddr_t *device, uint16_t role)
{
	...
	if (!session) {
		LOGE("Failed to connect to the local SDP server. %s(%d)",
	...
	if (!record) {
		LOGE("Failed to allocate service record %s(%d)",
							strerror(errno), errno);
		sdp_close(session);
		return -1;
	}
...
int bnep_sdp_search(bdaddr_t *src, bdaddr_t *dst, uint16_t service)
{
	...
	if (!s) {
		LOGE("Failed to connect to the SDP server. %s(%d)",
							strerror(errno), errno);
		return 0;
	}
...
int dun_sdp_register(bdaddr_t *device, uint8_t channel, int type)
{
	...
	record = sdp_record_alloc();
	if (!record) {
		LOGE("Failed to alloc service record");
		return -1;
	}
